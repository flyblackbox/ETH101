...
//This script is meant to facilitate organizational decision making.
  +Board members can submit proposals with a deadline, and all other members can vote to accept or reject.
  +If the proposal's score is positive when the deadline hits, it is archived as official ogranizational policy.
  +If the proposal's score is negative at the deadline, it is rejected and archived.
  +Each board member will have a voting weight which determines how many points they are able to commit to the proposal.
  +Board members can challenge a decision after the deadline is reached to force a revote (if their voteWeight exceeds winning score).
...
contract Boardroom is Stopable {
    ...
    function addMember(
                ...
                uint256 memberPassword, //To be entered by board member, kept secret from founder
                uint    voteWeight)
            isOwner(){
   ...
   function changeVoteWeight(uint newVoteWeight, address _memberAddress, bytes32 _membersPassword)
    ...
    function propose(
        bytes32 proposal,
        bytes32 _password,
        uint deadline){ //User inputs text for the proposal when calling this function as well as their password, and the number of blocks until deadline
          bytes32 passwordHash;
    ...
    function vote(bytes32 proposal, bytes32 password, bool voteTypeTrueUpFalseDown){
        ...
        proposals[proposal].score += voteWeight;
    }
